...
def fs_to_kp_pointing_model(pmodl_file):
    ...
    print "For interest, the phi value is %f" % phi
...
with h5py.File(name=args[0], mode='r+') as h5file:
    ...
    if timestamps[0] < 1488542661:  # Kludge for when the NTP on Reber was out of sync. After this it was fixed.
        timestamps = np.array(h5file["Data/Timestamps"])
        timestamps -= 171.0
        del h5file["Data/Timestamps"]
        print write_dataset("Timestamps", h5file["Data"], timestamps,
                            {"description": "Timestamps adjusted by 171 seconds to compensate for NTP error"})
    ...
    while True:  # because Python doesn't have do..while loops... :-/
        dummy = raw_input("Add default (dummy) environment sensor data? (y/n) ")
        try:
    ...
    enviro_timestamps = np.arange(rf_begin_time, rf_end_time, 10, dtype=np.float64)
    # The enviro sensor data needn't be high resolution, 10 seconds for dummy data should be okay.
    temperature_array = []
    pressure_array = []
    humidity_array = []
    windspeed_array = []
    ...
    print "Pos data start:\t\t%s UTC\nPos data end:\t\t%s UTC\nDuration:\t\t%s\n" % \
          (datetime.datetime.fromtimestamp(pos_begin_time).isoformat(),
           datetime.datetime.fromtimestamp(pos_end_time).isoformat(),
           csv_duration_str)
    ...
    try:
        ...
        print "CSV lower index: %.2f\tH5file lower index: %.2f" % \
              (csv_file["Timestamp"][pos_lower_index] / 1000.0, rf_begin_time)
        print "Position data commences %.2f seconds %s start of RF data." % \
              (np.abs(csv_file["Timestamp"][pos_lower_index] / 1000.0 - rf_begin_time),
    ...
    for i in range(0, len(timestamp_array), 20):  # Down-sample by a factor of 20
        ...
        azim_req_scan_pos_dset.append((float(csv_file["Timestamp"][pos_lower_index + i]) / 1000.0,
                                       np.degrees(antenna.pointing_model.reverse(
                                           np.radians(csv_file["Azim req position"][pos_lower_index + i]),
        ...
        azim_des_scan_pos_dset.append((float(csv_file["Timestamp"][pos_lower_index + i]) / 1000.0,
                                       np.degrees(antenna.pointing_model.reverse(
                                           np.radians(csv_file["Azim desired position"][pos_lower_index + i]),
